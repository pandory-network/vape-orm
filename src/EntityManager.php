...
use Vape\ORM\Exception\ConstraintViolationException;
use Vape\ORM\Exception\RelationException;
use Vape\ORM\Structure\Annotation\JoinTable;
use Vape\ORM\Structure\EntityStructure;
use Vape\ORM\Structure\Indexer\EntityIndexer;
use Vape\ORM\Structure\RelationManager;
use Vape\ORM\Structure\ValidationManager;
use Vape\ORM\Structure\Validator\Validator;
...
class EntityManager
{
    ...
    private function getEntityDataArray($entity, $strict = true, $only = array())
    {
        ...
        foreach ($columns as $column) {
            if (count($only) > 0 && !in_array($column->name, $only)) {
                continue;
            } // Ignore, we don't want this column!
